...
def convert_pointcloud2_bag_to_pcd(
    bag_file,
    output_dir,
    topic_name=None,  # Changed: Auto-detect if None
    ...
    # 如果未指定 topic，自动查找第一个 PointCloud2 topic
    if topic_name is None:
        for topic, msg, t in bag.read_messages():
            if msg._type == "sensor_msgs/PointCloud2":
                topic_name = topic
                print(f"[Bag] 自动检测到 topic: {topic_name}")
                break
...
if __name__ == "__main__":
    ...
    if msg_type == "PointCloud2":
        pcd_path = convert_pointcloud2_bag_to_pcd(
            ...
            topic_name="/ouster/points",  # Auto-detect
            pcd_name="ouster32_inten_ascii.pcd"
        )
    else:  # "CustomMsg"
        pcd_path = convert_livox_custom_bag_to_pcd(
            bag_file=bag_file,
            output_dir=output_dir,
            topic_name= None, #"/livox/lidar",
            pcd_name="livox_CustomMsg_inten_ascii.pcd"
        )
